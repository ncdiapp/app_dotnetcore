...
namespace DatabaseSchemaMrg.ProviderSchemaReaders
{
    class OracleSchemaReader : SchemaExtendedReader
    {
		...
		public static readonly  string  QueryTableAndSchemaOwner = @"select TABLE_NAME, OWNER from SYS.ALL_TABLES order by OWNER, TABLE_NAME ";
		public static readonly string  QueryAllTableColumns = @"select * FROM   SYS.ALL_TAB_COLUMNS";

		public static readonly string   QueryAllUser = @"select USERNAME from SYS.ALL_USERS order by USERNAME";

		public static readonly string QueryAllView = @"	select VIEW_NAME, OWNER from SYS.ALL_VIEWS order by OWNER, VIEW_NAME";

		public static readonly string QueryAllPackgeObject = @"select OBJECT_NAME, OWNER from SYS.ALL_OBJECTS where UPPER(OBJECT_TYPE) = 'PACKAGE' order by OWNER, OBJECT_NAME ";


		public static readonly  string QueryAllProcedure = @"select OBJECT_NAME, OWNER from SYS.ALL_OBJECTS where upper(OBJECT_TYPE) = upper('PROCEDURE') order by OWNER, OBJECT_NAME ";

		public static readonly string QueryProcedureColumn = @"select OWNER, OBJECT_NAME, ARGUMENT_NAME, DATA_TYPE, IN_OUT from SYS.ALL_ARGUMENTS order by OWNER, OBJECT_NAME, SEQUENCE";

		public static readonly  string QueryFunction = @"select OBJECT_NAME, OWNER from SYS.ALL_OBJECTS where upper(OBJECT_TYPE) = upper('FUNCTION') order by OWNER, OBJECT_NAME ";

		public static readonly string QueryTriger = @"select TRIGGER_NAME, OWNER from SYS.ALL_TRIGGERS order by OWNER, TRIGGER_NAME ";

		public static readonly string QueryIndex = @"select INDEX_NAME, TABLE_NAME, TABLE_OWNER from SYS.ALL_INDEXES order by TABLE_OWNER, TABLE_NAME, INDEX_NAME";
		...
		public static readonly string QueryCurrentUserAndSchema = @"SELECT sys_context('USERENV','SESSION_USER') as ""USER NAME"", sys_context('USERENV', 'CURRENT_SCHEMA') as ""CURRENT SCHEMA""  FROM dual";

		public static readonly string QueryCurrentUserSchema = @"SELECT  sys_context('USERENV', 'CURRENT_SCHEMA') as ""CURRENT SCHEMA""  FROM dual";
		...
		{
			DataTable dt = base.Tables();

#if NETFRAMEWORK
			dt = dt.Select("owner NOT IN " + BuitinOwner).CopyToDataTable();
#else
			// Filter rows manually for .NET Standard (CopyToDataTable not available)
			var filteredRows = dt.Select("owner NOT IN " + BuitinOwner);
			if (filteredRows.Length > 0)
			{
				dt = filteredRows[0].Table.Clone();
...
ORDER BY cons.table_name, cons.constraint_name";
					...
					using (DbDataAdapter da = Factory.CreateDataAdapter())
					{
						...
						const string sqlCommand = @"SELECT OWNER,
								...
								OR :TABLENAME   IS NULL)
								 AND COLUMN_NAME not like 'CDB$%'
								AND OWNER  NOT IN "+ BuitinOwner + @"
		...
		const string BuitinOwner = @"( 'SYS','AUDSYS','CTXSYS','APPQOSSYS','ANONYMOUS','CTXSYS','DBSNMP','EXFSYS','LBACSYS','MDSYS','MGMT_VIEW'
                                ,'DBSFWUSER','DVSYS','GSMADMIN_INTERNAL','OJVMSYS','OLAPSYS','ORDDATA','ORDSYS','OUTLN','SYSTEM','WMSYS','XDB'
                                )";
        ...
        private DataTable FindKeys(string tableName, string constraintType, DbConnection conn)
        {
			...
			{
				cmd.CommandText = sqlCommand;
				AddTableNameSchemaParameters(cmd,tableName);
				...
				DatabaseFixture.FillDataTable(dt, cmd);
			}
...
TRIGGER_NAME NOT IN ( SELECT object_name FROM USER_RECYCLEBIN )";
		...
		//protected override DataTable CommandForTableOld(string tableName, DbConnection conn, string collectionName, string sqlCommand)
  //      {
  //          DataTable dt = CreateDataTable(collectionName);
  ...
  //      }
        ...
        {
            var pk = databaseTable.FirstPrimaryKeyColumn;
            if (pk == null) return;
