...
namespace DatabaseSchemaMrg.Conversion
{
    class ColumnConverter
    {
		...
		private static void AddColumnDefault(DataRow row, string defaultKey, DatabaseColumn column)
		{
			if (string.IsNullOrEmpty(defaultKey)) return;
			string d = row[defaultKey].ToString();
			if (!string.IsNullOrEmpty(d)) column.DefaultValue = d.Trim(new[] { ' ', '\'', '=' });
		...
		private static bool CastToBoolean(DataRow row, string key)
		{
			var value = row[key];
		...
		private static void AddNullability(DataRow row, string nullableKey, DatabaseColumn column)
		{
			column.Nullable = CastToBoolean(row, nullableKey);
		}
