...
namespace DatabaseSchemaMrg
{
    ...
    public class SqlWriter
    {
        ...
        private HashSet<string> _ExcluedColumnUpdate = new HashSet<string>();
        ...
        public SqlWriter(DatabaseTable table, EmSqlType sqlType)
        {
            if (table == null)
        ...
        public SqlWriter(string databaseTableName, EmSqlType sqlType)
        {
        ...
        public string GenerateCreateTableQueryFromDataTable(DataTable dataTable)
        {
            string toReturn = "";
            string tableName = _DatabaseTableName;
            string sqlsc;
            ...
            if (_sqlType == EmSqlType.MySql)
            {
                /*
                   TINYTEXT:
                       Maximum size: 255 bytes
                       Maximum characters (assuming a single-byte character set): 255 characters
                       Use for short text, such as names, labels, or short descriptions.

                   TEXT:
                       Maximum size: 65,535 bytes (64 KB)
                       Maximum characters (assuming a single-byte character set): 65,535 characters
                       Suitable for longer text, such as paragraphs or articles.

                   MEDIUMTEXT:
                       Maximum size: 16,777,215 bytes (16 MB)
                       Maximum characters (assuming a single-byte character set): 16,777,215 characters
                       Ideal for storing even longer text, such as large documents or extensive textual content.

                   LONGTEXT:
                       Maximum size: 4,294,967,295 bytes (4 GB)
                       Maximum characters (assuming a single-byte character set): 4,294,967,295 characters
                       Used for very large text, like extremely long documents or textual data.
               */
                ...
                for (int i = 0; i < dataTable.Columns.Count; i++)
                {
                    ...
                    switch (columnType)
                    {
                        ...
                        default:
                            sqlsc += dataColumn.MaxLength == -1 ? ("TEXT") : (string.Format(" varchar({0}) ", dataColumn.MaxLength));
                            break;
                    }
                ...
                toReturn = sqlsc.Substring(0, sqlsc.Length - 1) + "\n)";
            }
            else if (_sqlType == EmSqlType.SqlServer)
            {
                ...
                for (int i = 0; i < dataTable.Columns.Count; i++)
                {
                    ...
                    switch (columnType)
                    {
                        ...
                        case "System.String": //65535 (nvarchr)
                        default:
                            sqlsc += string.Format(" nvarchar({0}) ", dataColumn.MaxLength == -1 ? "max" : dataColumn.MaxLength.ToString());
                            break;
                    }
                    if (dataColumn.AutoIncrement)
                        sqlsc += " IDENTITY(" + dataColumn.AutoIncrementSeed.ToString() + "," + dataColumn.AutoIncrementStep.ToString() + ") ";
            ...
            else if (_sqlType == EmSqlType.Oracle)
            {
                ...
                for (int i = 0; i < dataTable.Columns.Count; i++)
                {
                    ...
                    switch (columnType)
                    {
                        ...
                        case "System.String": //65535 (nvarchr)
                        default:
                            sqlsc += string.Format(" varchar({0}) ", dataColumn.MaxLength == -1 ? "4000" : dataColumn.MaxLength.ToString());
                            break;
                    }
                    if (dataColumn.AutoIncrement)
                        sqlsc += " IDENTITY(" + dataColumn.AutoIncrementSeed.ToString() + "," + dataColumn.AutoIncrementStep.ToString() + ") ";
        ...
        public string DropTableIfExist()
        {
            ...
            {
                case EmSqlType.MySql:
                    toReturn = $@"DROP TABLE IF EXISTS `{_DatabaseTableName}`";
                    break;
                case EmSqlType.SqlServer:
                    toReturn = $@"IF OBJECT_ID('{_DatabaseTableName}', 'U') IS NOT NULL   DROP TABLE {_DatabaseTableName} ";
                    break;
                case EmSqlType.Oracle:
                    toReturn = $@"DROP TABLE IF EXISTS `{_DatabaseTableName}`";
                    break;
        ...
        public string TruncateTable()
        {
            ...
            {
                case EmSqlType.MySql:
                    toReturn = $@"TRUNCATE TABLE `{_DatabaseTableName}`";
                    break;
                case EmSqlType.SqlServer:
                    toReturn = toReturn = $@"TRUNCATE TABLE [{_DatabaseTableName}]";
                    break;
        ...
        public string BuildUpdateStatement(Dictionary<string, string> dictUpdateTableNameAndAliasName, string setValues, string fromTables, string whereConditions)
        {
            string toReturn = "";

            string updateTables = string.Join(", ", dictUpdateTableNameAndAliasName.Keys.Select(o => "[" + o + "]").ToList());

            switch (_sqlType)
            {
                case EmSqlType.MySql:

                    foreach (var kv in dictUpdateTableNameAndAliasName)
                    {
                        if (!string.IsNullOrWhiteSpace(kv.Value))
                        {
                            setValues = setValues.Replace(kv.Key, kv.Value);
                        }
                    }

                    toReturn = $@"UPDATE {fromTables} 
                        SET {setValues}
                    ";
        ...
        public string CheckColumnExist(string columnName)
        {
            string query = "";
            if (_sqlType == EmSqlType.MySql)
            {
                query = $@"SHOW COLUMNS FROM {_DatabaseTableName} LIKE '{columnName}'";
            }
            else if (_sqlType == EmSqlType.SqlServer)
            {
                query = $@"select COL_LENGTH ('{_DatabaseTableName}', '{columnName}')";
            }

            return query;
        }

        public HashSet<string> ExcluedColumnUpdate
        {
            get
            {
                return _ExcluedColumnUpdate;
            }
        }
        ...
        public bool InStoredProcedure
        {
            ...
            set
            {
                if (_sqlType == EmSqlType.SqlServer) return; //always false
        ...
        public string EscapedName(string name)
        {
        ...
        private string AddPrimaryWhere()
        {
            string where = " WHERE ";
        ...
        public IList<string> NonPrimaryKeyColumns
        {
            get
            {
                ...
                foreach (DatabaseColumn column in _table.Columns)
                {
                    ...
                    if (_ExcluedColumnUpdate.Contains(name)) continue;
                    cols.Add(name);
                }
                return cols;
            }
        ...
        //public string[] GetColumnsExcludeAutoTimeStampAndComputed()
        //{
        //    var list = new List<string>();
        ...
        /// </summary>
        /// <returns></returns>
        public string SelectLimitedRecordFromQuery(string orgQuery, int recordLimit)
        {
            ...
            if (_sqlType == EmSqlType.MySql)
            {
                ...
                sb.Append(" FROM (" + orgQuery + ")  as orgResult ");
                sb.Append(" LIMIT " + recordLimit);
            }
            else
            {
                sb.AppendLine("SELECT TOP " + recordLimit + " * ");
                sb.Append(" FROM (" + orgQuery + ") as orgResult");
            }
        ...
        /// </summary>
        /// <returns></returns>
        public string SelectAllSql(int? recordLimit = null)
        {
            var sb = new StringBuilder();
            ...
            if (recordLimit.HasValue)
            {
                if (_sqlType == EmSqlType.MySql)
                {
                    ...
                    sb.Append(" LIMIT " + recordLimit.Value);
                }
                else
                {
                    sb.AppendLine("SELECT TOP " + recordLimit.Value);
                    sb.AppendLine(FormattedColumns(cols));
                    sb.Append(" FROM " + EscapedTableName);
                }
            }
        ...
        public string SelectPageSql()
        {
            ...
            else if (_sqlType == EmSqlType.PostgreSql)
            {
            ...
            else if (_sqlType == EmSqlType.SQLite)
            {
                sb.AppendLine(EscapedTableName);
            ...
            else if (_sqlType == EmSqlType.SqlServerCe)
            {
            ...
            else
            {
                ...
                if (_sqlType != EmSqlType.Oracle) sb.Append(" AS countedTable");
                sb.AppendLine(" WHERE");
                sb.AppendLine("   rowNumber > (" + ParameterName("pageSize") + " * (" + ParameterName("currentPage") + " - 1))");
                sb.AppendLine("   AND rowNumber <= (" + ParameterName("pageSize") + " * " + ParameterName("currentPage") + ")");
            }
...
FETCH NEXT @EndingRowNumber - @StartingRowNumber + 1 ROWS ONLY
        ...
        public string CountSql(string outputParameter)
        {
            ...
            if (_sqlType == EmSqlType.Oracle || _sqlType == EmSqlType.Db2)
        ...
        private string InsertSql(bool useOutputParameter, bool includeIdentityInInsert)
        {
            ...
            if (_table.HasAutoNumberColumn)
            {
                ...
                if (_sqlType == EmSqlType.Oracle && useOutputParameter)
                {
                ...
                else if (_sqlType == EmSqlType.MySql)
                {
                    sb.AppendLine(";");
                    if (useOutputParameter)
                    {
                ...
                else if (_sqlType == EmSqlType.PostgreSql)
                {
                    ...
                    var seq = _table.Name + "_" + ((_table.FirstPrimaryKeyColumn != null) ? _table.FirstPrimaryKeyColumn.Name : null) + "_seq";
                ...
                else if (_sqlType == EmSqlType.Db2)
                {
                    sb.AppendLine(";");
        ...
        /// </summary>
        /// <returns></returns>
        public string UpdateWithConcurrencySql()
        {
            var sb = new StringBuilder();
        ...
        /// </summary>
        /// <returns></returns>
        public string UpdateWithOutConcurrencySql()
        {
            var sb = new StringBuilder();
