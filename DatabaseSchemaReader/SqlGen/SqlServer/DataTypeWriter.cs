...
namespace DatabaseSchemaMrg.SqlGen.SqlServer
{
    /// <summary>
    /// Returns a datatype string (will convert common Oracle types to SqlServer)
    /// </summary>
    /// <remarks>
    /// <see cref="DatabaseSchemaMrg.SqlGen.SqlServerCe.DataTypeWriter"/> is derived from this for SqlServerCe
    /// </remarks>
    class DataTypeWriter : IDataTypeWriter
    {
        ...
        public DataTypeWriter(EmSqlType? originSqlType)
        {
            _originSqlType = originSqlType;
        }

        private readonly EmSqlType? _originSqlType;
        ...
        public virtual string WriteDataType(DatabaseColumn column)
        {
            ...
            if (dataType == "NTEXT" && length > 4000)
            {
                return "NTEXT";
            }
            if (dataType == "TEXT" && length > 4000)
            {
                return "TEXT";
            }

            if (dataType == "NVARCHAR" && length > 4000)
            {
                length = -1;
            }

            if (dataType == "VARCHAR" && length > 8000)
            {
                length = -1;
            }

            if (_originSqlType == EmSqlType.SqlServer || _originSqlType == EmSqlType.SqlServerCe)
            {
                if (dataType == "BINARY")
                {
