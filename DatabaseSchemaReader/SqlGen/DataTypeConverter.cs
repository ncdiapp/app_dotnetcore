    ...
    public static class DataTypeConverter
    {
        ...
        public static bool IsInteger(string dataType)
        {
            return (dataType == "INTEGER" ||
                    ...
                    dataType == "SMALLINT" ||
                    dataType == "TINYINT" ||
                    dataType == "bigint unsigned".ToUpper() ||
                    dataType == "int unsigned".ToUpper()
        ...
        public static void AddDataType(DatabaseColumn column)
        {
            ...
            var sqlType = EmSqlType.SqlServer;
            ...
            if (column.Table != null)
            {
                ...
                if (schema != null)
                {
                    ...
                    sqlType = Conversion.ProviderToSqlType.Convert(provider) ?? EmSqlType.SqlServer;
            ...
            if (IsInteger(dbType))
            {
                ...
                if (dbType == "BIGINT")
                {
                    netType = typeof(Int64).FullName;
                    var dataType = new DataType(dbType, netType);
                    column.DataType = dataType;
                    dataTypeList.Add(dataType);
                }
                else if (dbType.Contains("bigint unsigned".ToUpper()))
                {
                    netType = typeof(UInt64).FullName;
                    var dataType = new DataType(dbType, netType);
                    column.DataType = dataType;
                    dataTypeList.Add(dataType);
                }
                else if (dbType.Contains("int unsigned".ToUpper()))
                {
                    netType = typeof(Int32).FullName;
                    var dataType = new DataType(dbType, netType);
                    column.DataType = dataType;
                    dataTypeList.Add(dataType);
                }
