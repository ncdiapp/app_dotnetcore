...
using DatabaseSchemaMrg.Filters;
using DatabaseSchemaMrg.ProviderSchemaReaders;
...
namespace DatabaseSchemaMrg
{
    ...
    /// </remarks>
    public partial class DatabaseFixture : IDatabaseReader
    {
        ...
        private EmSqlType _SqlServerType;
        ...
        /// <summary>
        /// Optional session-level cache. When set, bulk DataTable loads are stored so that
        /// repeated calls to AllTables / AllStoredProcedures skip redundant DB round-trips.
        /// </summary>
        public ISchemaCache? Cache { get; set; }

        private DataTable GetCached(string key, Func<DataTable> loader)
        {
		...
		/// <summary>
		/// Gets all SqlServerType after Fxiture was setup
		/// </summary>
		public EmSqlType? SqlServerType
        {
            get
            {
		...
		public string CurrentOwner 
		{
			...
			get
			{
				return _schemaReader.CurrentUserSchemaOwner();
			}
		...
		/// <summary>
		/// Initializes a new instance of the <see cref="DatabaseFixture"/> class. For Oracle, use this overload.
		/// </summary>
		/// <param name="connectionString">The connection string.</param>
		/// <param name="sqlType">Name of the sqlType.</param>
		/// <param name="owner">The schema owner.</param>
		public DatabaseFixture(string connectionString, EmSqlType sqlType, string owner)
			: this(connectionString, sqlType)
		{
		...
		/// <summary>
		/// Initializes a new instance of the <see cref="DatabaseFixture"/> class. For Oracle, use the overload.
		/// try to avoid this constrctor in  App , onely for test purpose
		/// </summary>
		/// <param name="connectionString">The connection string.</param>
		/// <param name="providerName">Name of the provider.</param>
		public DatabaseFixture(string connectionString, string providerName)

		{
			_databaseSchema = new DatabaseSchema(connectionString, DatabaseSchema.GetSqlTypeByProvideName(providerName));
			_ProviderName = providerName;
			_SqlServerType = ProviderToSqlType.Convert(providerName).Value;
			_schemaReader = SchemaReaderFactory.Create(connectionString, _SqlServerType);
			_DbProviderFactory = Utilities.FactoryTools.GetFactory(providerName);
			_ConnectionString = connectionString;

		}

		/// <summary>
		/// Initializes a new instance of the <see cref="DatabaseFixture"/> class for one of the standard providers.
		/// </summary>
		/// <param name="connectionString">The connection string.</param>
		/// <param name="sqlType">Type of the SQL.</param>
		public DatabaseFixture(string connectionString, EmSqlType sqlType)
        {
			_databaseSchema = new DatabaseSchema(connectionString, sqlType);
			_schemaReader = SchemaReaderFactory.Create(connectionString, sqlType);
            _SqlServerType = sqlType;

			_DbProviderFactory = Utilities.FactoryTools.GetFactory(_databaseSchema.Provider);
			_ConnectionString = connectionString;
		}
        ...
        {
            DataTable tabs = GetCached("Tables", () => _schemaReader.Tables());
            ...
            DataTable ids = GetCached("IdentityColumns", () => _schemaReader.IdentityColumns(null));
            DataTable computeds = GetCached("ComputedColumns", () => _schemaReader.ComputedColumns(null));

            var tableDescriptions = new TableDescriptionConverter(GetCached("TableDescriptions", () => _schemaReader.TableDescription(null)));
            var columnDescriptions = new ColumnDescriptionConverter(GetCached("ColumnDescriptions", () => _schemaReader.ColumnDescription(null)));

            DataTable triggers = GetCached("Triggers", () => _schemaReader.Triggers(null));
        ...
        public IList<DatabaseStoredProcedure> AllStoredProcedures()
        {
            try
            {
                DataTable functions = GetCached("Functions", () => _schemaReader.Functions());
            ...
            DataTable dt = GetCached("StoredProcedures", () => _schemaReader.StoredProcedures());
            SchemaProcedureConverter.StoredProcedures(DatabaseSchema, dt);
            ...
            DatabaseSchema.Packages.AddRange(SchemaProcedureConverter.Packages(GetCached("Packages", () => _schemaReader.Packages())));
            ...
            DataTable args = GetCached("StoredProcedureArguments", () => _schemaReader.StoredProcedureArguments(null));
