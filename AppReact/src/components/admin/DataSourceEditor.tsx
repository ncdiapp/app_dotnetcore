...
const DataSourceEditor: React.FC<Props> = ({ sources, onChange, compact = false }) => {
  ...
  const add = () => {
    onChange([...sources, { alias: `src${sources.length + 1}`, type: 'sp', value: '' }]);
  };
          ...
          <div key={idx} className="flex items-center gap-1">
            ...
            {idx === 0 ? (
              <span className={`h-7 px-1.5 text-[10px] rounded-[4px] border flex items-center font-medium text-blue-500 shrink-0 ${t('border_mainContentSection')}`}
                title="Primary source (maps to {{header}}, {{rs1}})">
                PRIMARY
              </span>
            ) : (
              <input
                ...
                value={src.alias}
                onChange={e => update(idx, 'alias', e.target.value.replace(/\s/g, '_').toLowerCase())}
                placeholder="alias"
                title="Alias prefix for tokens: {{alias.Field}}, {{#each alias_rs1}}"
              />
            )}
      ...
      {sources.map((src, idx) => (
        <div key={idx} className={`rounded-[4px] border p-3 ${t('border_mainContentSection')} ${theme.mainContentSection}`}>
          <div className="flex items-center gap-2 mb-2">
            {idx === 0
              ? <span className="text-[10px] font-bold uppercase text-blue-500 bg-blue-50 px-1.5 py-0.5 rounded">Primary</span>
              : <span className="text-[10px] font-bold uppercase text-gray-400 px-1.5 py-0.5 rounded border border-gray-200">#{idx + 1}</span>}
            <span className={`text-xs flex-auto ${theme.label}`}>
              {idx === 0 ? 'Maps to {{header.…}}, {{rs1}}, {{rs2}}…' : `Maps to {{${src.alias}.…}}, {{#each ${src.alias}_rs1}}`}
            </span>
            {idx > 0 && (
          ...
          <div className="flex items-center gap-3">
            ...
            {idx > 0 && (
              <div className="flex items-center gap-1">
                <span className={`text-xs ${theme.label}`}>Alias:</span>
                <input
