...
const CompanyMenuRoleSetup: React.FC<Props> = ({ companyId, isEmbedded: _isEmbedded }) => {
  ...
  const loadData = useCallback(async () => {
    ...
    dispatch(setIsBusy());
    try {
      const list = await adminSvc.retrieveAppSecurityGroupDtoByGroupUsage(String(GROUP_USAGE_MENU_ROLE), true, '');
      const arr = Array.isArray(list) ? list : [];
      const udRoleList = arr.filter((r: any) => !r?.IsBuiltIn);
      rolesCV.sourceCollection = udRoleList;
      rolesCV.refresh();
  ...
  return (
    <div className={`w-full h-full flex flex-col rounded-t-md rounded-b-md overflow-hidden ${theme.mainContentSection}`}>
      <div className={`flex items-center justify-between px-3 py-2 mb-1 ${theme.mainContentSection}`}>
        <div className={`text-md font-semibold ${theme.title}`}>Menu Roles</div>
      ...
      {roleEditorGroupId !== '' && (
        <RoleEditorModal
          groupUsage={GROUP_USAGE_MENU_ROLE}
          companyId={companyId}
...
export default CompanyMenuRoleSetup;
