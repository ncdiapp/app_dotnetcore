-- ============================================================
-- SP_StyleSummary
-- Transaction 2246 (Style Summary) report data source
...
-- ============================================================
IF OBJECT_ID('dbo.SP_StyleSummary', 'P') IS NOT NULL
    DROP PROCEDURE dbo.SP_StyleSummary;
GO

CREATE PROCEDURE dbo.SP_StyleSummary
    @MainReferenceId    INT,
...
BEGIN
    ...
    INNER JOIN dbo.[Plm_Style_Summary] AS [u3] ON [u3].[ReferenceId] = [u1].[ReferenceId]
    ...
    LEFT JOIN [SourceERP].dbo.[tblComposition] AS [Composition_60] ON [Composition_60].[Composition_Id] = u3.[Composition]
    LEFT JOIN [SourceERP].dbo.[tblCountry] AS [Made_in_Country_61] ON [Made_in_Country_61].[Country_Id] = u3.[Made_in_Country]
    LEFT JOIN [TenantDB_PLM26].dbo.[Plm_pdmSecurityUserGroup] AS [Security_Group_3094_62] ON [Security_Group_3094_62].[GroupID] = u3.[Security_Group_3094]
    LEFT JOIN [TenantDB_PLM26].dbo.[Plm_pdmsecuritywebuser] AS [Designer_63] ON [Designer_63].[UserID] = u3.[Designer]
    LEFT JOIN [TenantDB_PLM26].dbo.[Plm_pdmSecurityUserGroup] AS [Security_Group_3098_64] ON [Security_Group_3098_64].[GroupID] = u3.[Security_Group_3098]
    LEFT JOIN [TenantDB_PLM26].dbo.[Plm_pdmsecuritywebuser] AS [Sourcing_65] ON [Sourcing_65].[UserID] = u3.[Sourcing]
    LEFT JOIN [TenantDB_PLM26].dbo.[Plm_pdmSecurityUserGroup] AS [Security_Group_3950_66] ON [Security_Group_3950_66].[GroupID] = u3.[Security_Group_3950]
    LEFT JOIN [TenantDB_PLM26].dbo.[Plm_pdmsecuritywebuser] AS [Technical_Manager_67] ON [Technical_Manager_67].[UserID] = u3.[Technical_Manager]
    LEFT JOIN [TenantDB_PLM26].dbo.[Plm_pdmsecuritywebuser] AS [Coordinator_68] ON [Coordinator_68].[UserID] = u3.[Coordinator]
    LEFT JOIN [TenantDB_PLM26].dbo.[Plm_pdmsecuritywebuser] AS [QC_Team_69] ON [QC_Team_69].[UserID] = u3.[QC_Team]
    LEFT JOIN dbo.AppEntitySimpleListValue AS [Release_to_Manufacturer_70] ON [Release_to_Manufacturer_70].EntityInfoID = 4813 AND [Release_to_Manufacturer_70].InternalKey = u3.[Release_to_Manufacturer]
    LEFT JOIN [TenantDB_PLM26].dbo.[Plm_pdmSecurityUserGroup] AS [Security_Group_5096_71] ON [Security_Group_5096_71].[GroupID] = u3.[Security_Group_5096]
    LEFT JOIN [TenantDB_PLM26].dbo.[Plm_pdmSecurityUserGroup] AS [Security_Group_5097_72] ON [Security_Group_5097_72].[GroupID] = u3.[Security_Group_5097]
    LEFT JOIN dbo.AppEntitySimpleListValue AS [Tech_Pack_Send_to_Vendor_73] ON [Tech_Pack_Send_to_Vendor_73].EntityInfoID = 4813 AND [Tech_Pack_Send_to_Vendor_73].InternalKey = u3.[Tech_Pack_Send_to_Vendor]
    LEFT JOIN dbo.AppEntitySimpleListValue AS [Fiber1CB_7082_74] ON [Fiber1CB_7082_74].EntityInfoID = 4645 AND [Fiber1CB_7082_74].InternalKey = u3.[Fiber1CB_7082]
    LEFT JOIN dbo.AppEntitySimpleListValue AS [Fiber2CB_7083_75] ON [Fiber2CB_7083_75].EntityInfoID = 4645 AND [Fiber2CB_7083_75].InternalKey = u3.[Fiber2CB_7083]
    LEFT JOIN dbo.AppEntitySimpleListValue AS [Fiber3CB_7084_76] ON [Fiber3CB_7084_76].EntityInfoID = 4645 AND [Fiber3CB_7084_76].InternalKey = u3.[Fiber3CB_7084]
    LEFT JOIN [SourceERP].dbo.[tblComposition] AS [CompositionDDL_7090_77] ON [CompositionDDL_7090_77].[Composition_Id] = u3.[CompositionDDL_7090]
    LEFT JOIN dbo.AppEntitySimpleListValue AS [Fiber4CB_7102_78] ON [Fiber4CB_7102_78].EntityInfoID = 4645 AND [Fiber4CB_7102_78].InternalKey = u3.[Fiber4CB_7102]
    LEFT JOIN dbo.AppEntitySimpleListValue AS [Fiber5CB_7103_79] ON [Fiber5CB_7103_79].EntityInfoID = 4645 AND [Fiber5CB_7103_79].InternalKey = u3.[Fiber5CB_7103]
    LEFT JOIN dbo.AppEntitySimpleListValue AS [Designer_2_80] ON [Designer_2_80].EntityInfoID = 4628 AND [Designer_2_80].InternalKey = u3.[Designer_2]
    LEFT JOIN dbo.AppEntitySimpleListValue AS [Designer_1_81] ON [Designer_1_81].EntityInfoID = 4628 AND [Designer_1_81].InternalKey = u3.[Designer_1]
    LEFT JOIN dbo.AppEntitySimpleListValue AS [Fiber6CB_82] ON [Fiber6CB_82].EntityInfoID = 4645 AND [Fiber6CB_82].InternalKey = u3.[Fiber6CB]
    ...
    BEGIN
        ...
        CAST(NULL AS NVARCHAR(MAX)) AS [PlmStyleSummary_ReferenceId],
        CAST(NULL AS NVARCHAR(MAX)) AS [MainFabricComposition],
        CAST(NULL AS NVARCHAR(MAX)) AS [MadeinCountry],
        CAST(NULL AS NVARCHAR(MAX)) AS [SecurityGroup3094],
        CAST(NULL AS NVARCHAR(MAX)) AS [Designer],
        CAST(NULL AS NVARCHAR(MAX)) AS [SecurityGroup3098],
        CAST(NULL AS NVARCHAR(MAX)) AS [Sourcing],
        CAST(NULL AS NVARCHAR(MAX)) AS [SecurityGroup3950],
        CAST(NULL AS NVARCHAR(MAX)) AS [TechnicalManager],
        CAST(NULL AS NVARCHAR(MAX)) AS [Coordinator],
        CAST(NULL AS NVARCHAR(MAX)) AS [QCTeam],
        CAST(NULL AS NVARCHAR(MAX)) AS [ReleasetoManufacturer],
        CAST(NULL AS NVARCHAR(MAX)) AS [SecurityGroup5096],
        CAST(NULL AS NVARCHAR(MAX)) AS [SecurityGroup5097],
        CAST(NULL AS NVARCHAR(MAX)) AS [Comments],
        CAST(NULL AS NVARCHAR(MAX)) AS [TechPackSendtoVendor],
        CAST(NULL AS NVARCHAR(MAX)) AS [TechPackSentDate],
        CAST(NULL AS NVARCHAR(MAX)) AS [RRP],
        CAST(NULL AS NVARCHAR(MAX)) AS [TargetCostFOB],
        CAST(NULL AS NVARCHAR(MAX)) AS [TargetMargin],
        CAST(NULL AS NVARCHAR(MAX)) AS [TargetLanded],
        CAST(NULL AS NVARCHAR(MAX)) AS [Drop],
        CAST(NULL AS NVARCHAR(MAX)) AS [LaunchMonth],
        CAST(NULL AS NVARCHAR(MAX)) AS [OrderMonth],
        CAST(NULL AS NVARCHAR(MAX)) AS [ShipWindowStart],
        CAST(NULL AS NVARCHAR(MAX)) AS [ShipWindowEnd],
        CAST(NULL AS NVARCHAR(MAX)) AS [DateShipping1],
        CAST(NULL AS NVARCHAR(MAX)) AS [ETD],
        CAST(NULL AS NVARCHAR(MAX)) AS [FloorsetDate],
        CAST(NULL AS NVARCHAR(MAX)) AS [CancelByDate],
        CAST(NULL AS NVARCHAR(MAX)) AS [EmbellishmentAWTechniques],
        CAST(NULL AS NVARCHAR(MAX)) AS [WashThread],
        CAST(NULL AS NVARCHAR(MAX)) AS [PricingComment],
        CAST(NULL AS NVARCHAR(MAX)) AS [ReleaseDay],
        CAST(NULL AS NVARCHAR(MAX)) AS [Comp17079],
        CAST(NULL AS NVARCHAR(MAX)) AS [Comp27080],
        CAST(NULL AS NVARCHAR(MAX)) AS [Comp37081],
        CAST(NULL AS NVARCHAR(MAX)) AS [Fiber1CB7082],
        CAST(NULL AS NVARCHAR(MAX)) AS [Fiber2CB7083],
        CAST(NULL AS NVARCHAR(MAX)) AS [Fiber3CB7084],
        CAST(NULL AS NVARCHAR(MAX)) AS [Fiber1IB7085],
        CAST(NULL AS NVARCHAR(MAX)) AS [Fiber2IB7086],
        CAST(NULL AS NVARCHAR(MAX)) AS [Fiber3IB7087],
        CAST(NULL AS NVARCHAR(MAX)) AS [Compok7088],
        CAST(NULL AS NVARCHAR(MAX)) AS [TotalComposition7089],
        CAST(NULL AS NVARCHAR(MAX)) AS [CompositionDDL7090],
        CAST(NULL AS NVARCHAR(MAX)) AS [ValidSelection7091],
        CAST(NULL AS NVARCHAR(MAX)) AS [CompositionTXT7092],
        CAST(NULL AS NVARCHAR(MAX)) AS [comp1ok7093],
        CAST(NULL AS NVARCHAR(MAX)) AS [comp2ok7094],
        CAST(NULL AS NVARCHAR(MAX)) AS [comp3ok7095],
        CAST(NULL AS NVARCHAR(MAX)) AS [Comp1Tx7096],
        CAST(NULL AS NVARCHAR(MAX)) AS [Comp2Tx7097],
        CAST(NULL AS NVARCHAR(MAX)) AS [Comp3Tx7098],
        CAST(NULL AS NVARCHAR(MAX)) AS [PercentChk7099],
        CAST(NULL AS NVARCHAR(MAX)) AS [Comp47100],
        CAST(NULL AS NVARCHAR(MAX)) AS [Comp57101],
        CAST(NULL AS NVARCHAR(MAX)) AS [Fiber4CB7102],
        CAST(NULL AS NVARCHAR(MAX)) AS [Fiber5CB7103],
        CAST(NULL AS NVARCHAR(MAX)) AS [Fiber4IB7104],
        CAST(NULL AS NVARCHAR(MAX)) AS [Fiber5IB7105],
        CAST(NULL AS NVARCHAR(MAX)) AS [comp4ok7106],
        CAST(NULL AS NVARCHAR(MAX)) AS [comp5ok7107],
        CAST(NULL AS NVARCHAR(MAX)) AS [Comp4Tx7108],
        CAST(NULL AS NVARCHAR(MAX)) AS [Comp5Tx7109],
        CAST(NULL AS NVARCHAR(MAX)) AS [PlmStyleSummary_FabricCode],
        CAST(NULL AS NVARCHAR(MAX)) AS [MainFabricComposition7159],
        CAST(NULL AS NVARCHAR(MAX)) AS [Weight7169],
        CAST(NULL AS NVARCHAR(MAX)) AS [UOM7170],
        CAST(NULL AS NVARCHAR(MAX)) AS [Designer2],
        CAST(NULL AS NVARCHAR(MAX)) AS [Designer1],
        CAST(NULL AS NVARCHAR(MAX)) AS [Calc7220],
        CAST(NULL AS NVARCHAR(MAX)) AS [MainFabric7221],
        CAST(NULL AS NVARCHAR(MAX)) AS [Calc7222],
        CAST(NULL AS NVARCHAR(MAX)) AS [Weight7223],
        CAST(NULL AS NVARCHAR(MAX)) AS [UOM7224],
        CAST(NULL AS NVARCHAR(MAX)) AS [Comp17225],
        CAST(NULL AS NVARCHAR(MAX)) AS [Comp27226],
        CAST(NULL AS NVARCHAR(MAX)) AS [Comp37227],
        CAST(NULL AS NVARCHAR(MAX)) AS [Fiber1CB7228],
        CAST(NULL AS NVARCHAR(MAX)) AS [Fiber2CB7229],
        CAST(NULL AS NVARCHAR(MAX)) AS [Fiber3CB7230],
        CAST(NULL AS NVARCHAR(MAX)) AS [Fiber1IB7231],
        CAST(NULL AS NVARCHAR(MAX)) AS [Fiber2IB7232],
        CAST(NULL AS NVARCHAR(MAX)) AS [Fiber3IB7233],
        CAST(NULL AS NVARCHAR(MAX)) AS [Compok7234],
        CAST(NULL AS NVARCHAR(MAX)) AS [TotalComposition7235],
        CAST(NULL AS NVARCHAR(MAX)) AS [CompositionDDL7236],
        CAST(NULL AS NVARCHAR(MAX)) AS [ValidSelection7237],
        CAST(NULL AS NVARCHAR(MAX)) AS [CompositionTXT7238],
        CAST(NULL AS NVARCHAR(MAX)) AS [comp1ok7239],
        CAST(NULL AS NVARCHAR(MAX)) AS [comp2ok7240],
        CAST(NULL AS NVARCHAR(MAX)) AS [comp3ok7241],
        CAST(NULL AS NVARCHAR(MAX)) AS [Comp1Tx7242],
        CAST(NULL AS NVARCHAR(MAX)) AS [Comp2Tx7243],
        CAST(NULL AS NVARCHAR(MAX)) AS [Comp3Tx7244],
        CAST(NULL AS NVARCHAR(MAX)) AS [PercentChk7245],
        CAST(NULL AS NVARCHAR(MAX)) AS [Comp47246],
        CAST(NULL AS NVARCHAR(MAX)) AS [Comp57247],
        CAST(NULL AS NVARCHAR(MAX)) AS [Fiber4CB7248],
        CAST(NULL AS NVARCHAR(MAX)) AS [Fiber5CB7249],
        CAST(NULL AS NVARCHAR(MAX)) AS [Fiber4IB7250],
        CAST(NULL AS NVARCHAR(MAX)) AS [Fiber5IB7251],
        CAST(NULL AS NVARCHAR(MAX)) AS [comp4ok7252],
        CAST(NULL AS NVARCHAR(MAX)) AS [comp5ok7253],
        CAST(NULL AS NVARCHAR(MAX)) AS [Comp4Tx7254],
        CAST(NULL AS NVARCHAR(MAX)) AS [Comp5Tx7255],
        CAST(NULL AS NVARCHAR(MAX)) AS [MainFabricComposition7256],
        CAST(NULL AS NVARCHAR(MAX)) AS [PlmStyleSummary_FabricCode_2],
        CAST(NULL AS NVARCHAR(MAX)) AS [Weight],
        CAST(NULL AS NVARCHAR(MAX)) AS [UOM],
        CAST(NULL AS NVARCHAR(MAX)) AS [MainFabric10],
        CAST(NULL AS NVARCHAR(MAX)) AS [MainWeight10],
        CAST(NULL AS NVARCHAR(MAX)) AS [MainUOM10],
        CAST(NULL AS NVARCHAR(MAX)) AS [MainFabric20],
        CAST(NULL AS NVARCHAR(MAX)) AS [MainWeight20],
        CAST(NULL AS NVARCHAR(MAX)) AS [MainUOM20],
        CAST(NULL AS NVARCHAR(MAX)) AS [TotalComposition10],
        CAST(NULL AS NVARCHAR(MAX)) AS [TotalComposition20],
        CAST(NULL AS NVARCHAR(MAX)) AS [FinalTotalComposition],
        CAST(NULL AS NVARCHAR(MAX)) AS [UOMID10],
        CAST(NULL AS NVARCHAR(MAX)) AS [Compositions],
        CAST(NULL AS NVARCHAR(MAX)) AS [Comp6],
        CAST(NULL AS NVARCHAR(MAX)) AS [Fiber6CB],
        CAST(NULL AS NVARCHAR(MAX)) AS [Fiber6IB],
        CAST(NULL AS NVARCHAR(MAX)) AS [comp6ok],
        CAST(NULL AS NVARCHAR(MAX)) AS [Comp6Tx],
        CAST(NULL AS NVARCHAR(MAX)) AS [PlmStyleSummary_TypeGroup],
        CAST(NULL AS NVARCHAR(MAX)) AS [isOuterwearSwim],
        CAST(NULL AS NVARCHAR(MAX)) AS [OSCompositions],
        CAST(NULL AS NVARCHAR(MAX)) AS [UOMID20],
        CAST(NULL AS NVARCHAR(MAX)) AS [FinalUOMID],
        CAST(NULL AS NVARCHAR(MAX)) AS [SetCompositions];
        RETURN;
    END
    ...
    SELECT
        ...
        u3.[ReferenceId] AS [PlmStyleSummary_ReferenceId],
        [Composition_60].[CompositionDesc_English] AS [MainFabricComposition],
        [Made_in_Country_61].[Name] AS [MadeinCountry],
        [Security_Group_3094_62].[GroupName] AS [SecurityGroup3094],
        [Designer_63].[LoginName] AS [Designer],
        [Security_Group_3098_64].[GroupName] AS [SecurityGroup3098],
        [Sourcing_65].[LoginName] AS [Sourcing],
        [Security_Group_3950_66].[GroupName] AS [SecurityGroup3950],
        [Technical_Manager_67].[LoginName] AS [TechnicalManager],
        [Coordinator_68].[LoginName] AS [Coordinator],
        [QC_Team_69].[LoginName] AS [QCTeam],
        [Release_to_Manufacturer_70].Code AS [ReleasetoManufacturer],
        [Security_Group_5096_71].[GroupName] AS [SecurityGroup5096],
        [Security_Group_5097_72].[GroupName] AS [SecurityGroup5097],
        u3.[Comments] AS [Comments],
        [Tech_Pack_Send_to_Vendor_73].Code AS [TechPackSendtoVendor],
        u3.[Tech_Pack_Sent_Date] AS [TechPackSentDate],
        u3.[RRP] AS [RRP],
        u3.[Target_Cost_FOB] AS [TargetCostFOB],
        u3.[Target_Margin] AS [TargetMargin],
        u3.[Target_Landed] AS [TargetLanded],
        u3.[Drop] AS [Drop],
        u3.[Launch_Month] AS [LaunchMonth],
        u3.[Order_Month] AS [OrderMonth],
        u3.[Ship_Window_Start] AS [ShipWindowStart],
        u3.[Ship_Window_End] AS [ShipWindowEnd],
        u3.[Date_Shipping_1] AS [DateShipping1],
        u3.[ETD] AS [ETD],
        u3.[Floorset_Date] AS [FloorsetDate],
        u3.[Cancel_By_Date] AS [CancelByDate],
        u3.[Embellishment_A_W_Techniques] AS [EmbellishmentAWTechniques],
        u3.[Wash_Thread] AS [WashThread],
        u3.[Pricing_Comment] AS [PricingComment],
        u3.[Release_Day] AS [ReleaseDay],
        u3.[Comp1__7079] AS [Comp17079],
        u3.[Comp2__7080] AS [Comp27080],
        u3.[Comp3__7081] AS [Comp37081],
        [Fiber1CB_7082_74].Code AS [Fiber1CB7082],
        [Fiber2CB_7083_75].Code AS [Fiber2CB7083],
        [Fiber3CB_7084_76].Code AS [Fiber3CB7084],
        u3.[Fiber1IB_7085] AS [Fiber1IB7085],
        u3.[Fiber2IB_7086] AS [Fiber2IB7086],
        u3.[Fiber3IB_7087] AS [Fiber3IB7087],
        CASE WHEN u3.[Comp_ok_7088] IN ('1','Y','True','true') THEN 'Yes' WHEN u3.[Comp_ok_7088] IN ('0','N','False','false') THEN 'No' ELSE CAST(u3.[Comp_ok_7088] AS NVARCHAR(MAX)) END AS [Compok7088],
        u3.[Total_Composition_7089] AS [TotalComposition7089],
        [CompositionDDL_7090_77].[CompositionDesc_English] AS [CompositionDDL7090],
        CASE WHEN u3.[Valid_Selection_7091] IN ('1','Y','True','true') THEN 'Yes' WHEN u3.[Valid_Selection_7091] IN ('0','N','False','false') THEN 'No' ELSE CAST(u3.[Valid_Selection_7091] AS NVARCHAR(MAX)) END AS [ValidSelection7091],
        u3.[CompositionTXT_7092] AS [CompositionTXT7092],
        CASE WHEN u3.[comp1ok_7093] IN ('1','Y','True','true') THEN 'Yes' WHEN u3.[comp1ok_7093] IN ('0','N','False','false') THEN 'No' ELSE CAST(u3.[comp1ok_7093] AS NVARCHAR(MAX)) END AS [comp1ok7093],
        CASE WHEN u3.[comp2ok_7094] IN ('1','Y','True','true') THEN 'Yes' WHEN u3.[comp2ok_7094] IN ('0','N','False','false') THEN 'No' ELSE CAST(u3.[comp2ok_7094] AS NVARCHAR(MAX)) END AS [comp2ok7094],
        CASE WHEN u3.[comp3ok_7095] IN ('1','Y','True','true') THEN 'Yes' WHEN u3.[comp3ok_7095] IN ('0','N','False','false') THEN 'No' ELSE CAST(u3.[comp3ok_7095] AS NVARCHAR(MAX)) END AS [comp3ok7095],
        u3.[Comp1_Tx_7096] AS [Comp1Tx7096],
        u3.[Comp2_Tx_7097] AS [Comp2Tx7097],
        u3.[Comp3_Tx_7098] AS [Comp3Tx7098],
        CASE WHEN u3.[Percent_Chk_7099] IN ('1','Y','True','true') THEN 'Yes' WHEN u3.[Percent_Chk_7099] IN ('0','N','False','false') THEN 'No' ELSE CAST(u3.[Percent_Chk_7099] AS NVARCHAR(MAX)) END AS [PercentChk7099],
        u3.[Comp4__7100] AS [Comp47100],
        u3.[Comp5__7101] AS [Comp57101],
        [Fiber4CB_7102_78].Code AS [Fiber4CB7102],
        [Fiber5CB_7103_79].Code AS [Fiber5CB7103],
        u3.[Fiber4IB_7104] AS [Fiber4IB7104],
        u3.[Fiber5IB_7105] AS [Fiber5IB7105],
        CASE WHEN u3.[comp4ok_7106] IN ('1','Y','True','true') THEN 'Yes' WHEN u3.[comp4ok_7106] IN ('0','N','False','false') THEN 'No' ELSE CAST(u3.[comp4ok_7106] AS NVARCHAR(MAX)) END AS [comp4ok7106],
        CASE WHEN u3.[comp5ok_7107] IN ('1','Y','True','true') THEN 'Yes' WHEN u3.[comp5ok_7107] IN ('0','N','False','false') THEN 'No' ELSE CAST(u3.[comp5ok_7107] AS NVARCHAR(MAX)) END AS [comp5ok7107],
        u3.[Comp4_Tx_7108] AS [Comp4Tx7108],
        u3.[Comp5_Tx_7109] AS [Comp5Tx7109],
        u3.[Main_Fabric_7156] AS [PlmStyleSummary_FabricCode],
        u3.[Main_Fabric_Composition_7159] AS [MainFabricComposition7159],
        u3.[Weight_7169] AS [Weight7169],
        u3.[UOM_7170] AS [UOM7170],
        [Designer_2_80].Code AS [Designer2],
        [Designer_1_81].Code AS [Designer1],
        u3.[Calc_7220] AS [Calc7220],
        u3.[Main_Fabric_7221] AS [MainFabric7221],
        u3.[Calc_7222] AS [Calc7222],
        u3.[Weight_7223] AS [Weight7223],
        u3.[UOM_7224] AS [UOM7224],
        u3.[Comp1__7225] AS [Comp17225],
        u3.[Comp2__7226] AS [Comp27226],
        u3.[Comp3__7227] AS [Comp37227],
        u3.[Fiber1CB_7228] AS [Fiber1CB7228],
        u3.[Fiber2CB_7229] AS [Fiber2CB7229],
        u3.[Fiber3CB_7230] AS [Fiber3CB7230],
        u3.[Fiber1IB_7231] AS [Fiber1IB7231],
        u3.[Fiber2IB_7232] AS [Fiber2IB7232],
        u3.[Fiber3IB_7233] AS [Fiber3IB7233],
        CASE WHEN u3.[Comp_ok_7234] IN ('1','Y','True','true') THEN 'Yes' WHEN u3.[Comp_ok_7234] IN ('0','N','False','false') THEN 'No' ELSE CAST(u3.[Comp_ok_7234] AS NVARCHAR(MAX)) END AS [Compok7234],
        u3.[Total_Composition_7235] AS [TotalComposition7235],
        u3.[CompositionDDL_7236] AS [CompositionDDL7236],
        CASE WHEN u3.[Valid_Selection_7237] IN ('1','Y','True','true') THEN 'Yes' WHEN u3.[Valid_Selection_7237] IN ('0','N','False','false') THEN 'No' ELSE CAST(u3.[Valid_Selection_7237] AS NVARCHAR(MAX)) END AS [ValidSelection7237],
        u3.[CompositionTXT_7238] AS [CompositionTXT7238],
        CASE WHEN u3.[comp1ok_7239] IN ('1','Y','True','true') THEN 'Yes' WHEN u3.[comp1ok_7239] IN ('0','N','False','false') THEN 'No' ELSE CAST(u3.[comp1ok_7239] AS NVARCHAR(MAX)) END AS [comp1ok7239],
        CASE WHEN u3.[comp2ok_7240] IN ('1','Y','True','true') THEN 'Yes' WHEN u3.[comp2ok_7240] IN ('0','N','False','false') THEN 'No' ELSE CAST(u3.[comp2ok_7240] AS NVARCHAR(MAX)) END AS [comp2ok7240],
        CASE WHEN u3.[comp3ok_7241] IN ('1','Y','True','true') THEN 'Yes' WHEN u3.[comp3ok_7241] IN ('0','N','False','false') THEN 'No' ELSE CAST(u3.[comp3ok_7241] AS NVARCHAR(MAX)) END AS [comp3ok7241],
        u3.[Comp1_Tx_7242] AS [Comp1Tx7242],
        u3.[Comp2_Tx_7243] AS [Comp2Tx7243],
        u3.[Comp3_Tx_7244] AS [Comp3Tx7244],
        CASE WHEN u3.[Percent_Chk_7245] IN ('1','Y','True','true') THEN 'Yes' WHEN u3.[Percent_Chk_7245] IN ('0','N','False','false') THEN 'No' ELSE CAST(u3.[Percent_Chk_7245] AS NVARCHAR(MAX)) END AS [PercentChk7245],
        u3.[Comp4__7246] AS [Comp47246],
        u3.[Comp5__7247] AS [Comp57247],
        u3.[Fiber4CB_7248] AS [Fiber4CB7248],
        u3.[Fiber5CB_7249] AS [Fiber5CB7249],
        u3.[Fiber4IB_7250] AS [Fiber4IB7250],
        u3.[Fiber5IB_7251] AS [Fiber5IB7251],
        CASE WHEN u3.[comp4ok_7252] IN ('1','Y','True','true') THEN 'Yes' WHEN u3.[comp4ok_7252] IN ('0','N','False','false') THEN 'No' ELSE CAST(u3.[comp4ok_7252] AS NVARCHAR(MAX)) END AS [comp4ok7252],
        CASE WHEN u3.[comp5ok_7253] IN ('1','Y','True','true') THEN 'Yes' WHEN u3.[comp5ok_7253] IN ('0','N','False','false') THEN 'No' ELSE CAST(u3.[comp5ok_7253] AS NVARCHAR(MAX)) END AS [comp5ok7253],
        u3.[Comp4_Tx_7254] AS [Comp4Tx7254],
        u3.[Comp5_Tx_7255] AS [Comp5Tx7255],
        u3.[Main_Fabric_Composition_7256] AS [MainFabricComposition7256],
        u3.[Final_Main_Fabric] AS [PlmStyleSummary_FabricCode_2],
        u3.[Final_Weight] AS [Weight],
        u3.[Final_UOM] AS [UOM],
        u3.[Main_Fabric_10] AS [MainFabric10],
        u3.[Main_Weight_10] AS [MainWeight10],
        u3.[Main_UOM_10] AS [MainUOM10],
        u3.[Main_Fabric_20] AS [MainFabric20],
        u3.[Main_Weight_20] AS [MainWeight20],
        u3.[Main_UOM_20] AS [MainUOM20],
        u3.[Total_Composition_10] AS [TotalComposition10],
        u3.[Total_Composition_20] AS [TotalComposition20],
        u3.[Final_Total_Composition] AS [FinalTotalComposition],
        u3.[UOM_ID_10] AS [UOMID10],
        u3.[Compositions] AS [Compositions],
        u3.[Comp6] AS [Comp6],
        [Fiber6CB_82].Code AS [Fiber6CB],
        u3.[Fiber6IB] AS [Fiber6IB],
        CASE WHEN u3.[comp6ok] IN ('1','Y','True','true') THEN 'Yes' WHEN u3.[comp6ok] IN ('0','N','False','false') THEN 'No' ELSE CAST(u3.[comp6ok] AS NVARCHAR(MAX)) END AS [comp6ok],
        u3.[Comp6_Tx] AS [Comp6Tx],
        u3.[Type_Group] AS [PlmStyleSummary_TypeGroup],
        u3.[is_Outerwear_Swim] AS [isOuterwearSwim],
        u3.[O_S_Compositions] AS [OSCompositions],
        u3.[UOM_ID_20] AS [UOMID20],
        u3.[Final_UOM_ID] AS [FinalUOMID],
        u3.[Set_Compositions] AS [SetCompositions]
    FROM dbo.[Plm_ReferenceBasicInfo] AS [u1]
    INNER JOIN dbo.[Plm_Style_Header] AS [u2] ON [u2].[ReferenceId] = [u1].[ReferenceId]
    INNER JOIN dbo.[Plm_Style_Summary] AS [u3] ON [u3].[ReferenceId] = [u1].[ReferenceId]
    LEFT JOIN [SourceERP].dbo.[tblProductClass] AS [Classification_1] ON [Classification_1].[ProductClassID] = u2.[Classification]
