-- =============================================================================
-- PLM BOM Colorway staging cleanup (drop wide host columns after grandchild import)
-- Template: source/PlmDw_CleanupBomColorwayStaging.sql
-- Deliverable: output/{templateId}/6_PlmDw_CleanupBomColorwayStaging.sql
--
-- EXECUTION ORDER:
...
--   5. 5_PlmDw_ImportBomColorwayGrandchild.sql
--   6. 6_PlmDw_CleanupBomColorwayStaging.sql   (this file)
--
-- Drops host Colorway_N / ImageN staging columns and BomColorwaySlot FieldMapping rows.
...
-- <<< USER SETTINGS — generator patches from dwTabImportConfig.json >>>
DECLARE @TablePrefix     NVARCHAR(32)  = N'Plm_';
DECLARE @HostAppTable    NVARCHAR(128) = NULL;   -- e.g. Artwork_BOM_prod
DECLARE @PlmTabId        INT           = NULL;   -- host tab id (e.g. 4246) for Tab_{id} integration lookup
...
BEGIN
    ...
    IF @Cnt = 0
    BEGIN
        RAISERROR(N'Grandchild table dbo.%s is empty — aborting cleanup. Set @RequireGrandchildRows = 0 to force.', 16, 1, @GrandchildTable);
        RETURN;
    END
END
