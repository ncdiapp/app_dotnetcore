...
"@.Trim())
    ...
    & $add ''
    & $add '-- ============================================================================='
    & $add '-- QX1: QcSelectedSizes + View_TchpSimpleQcSelectedSizes (Simple QC pivot domain)'
    & $add '-- Keep identical to Document/Design/POM_Grading_QC_NewSchema.sql'
    & $add '-- ============================================================================='
    ...
    & $add '    PRINT N''Added TchpStyleSpec.QcSelectedSizes'';'
    & $add 'END'
    & $add 'GO'
    & $add ''
    & $add 'CREATE OR ALTER VIEW dbo.View_TchpSimpleQcSelectedSizes'
    & $add 'AS'
    & $add 'SELECT'
    & $add '    ss.StyleSpecId,'
    & $add '    ss.SizeRunId,'
    ...
    & $add '    srs.IsActive,'
    & $add '    CASE'
    & $add '        WHEN NULLIF(LTRIM(RTRIM(ss.QcSelectedSizes)), N'''') IS NULL THEN 1'
    & $add '        WHEN EXISTS ('
    & $add '            SELECT 1'
    & $add '            FROM STRING_SPLIT(REPLACE(ss.QcSelectedSizes, N''|'', N'',''), N'','') AS tok'
    & $add '            WHERE TRY_CONVERT(INT, LTRIM(RTRIM(tok.[value]))) = srs.SizeRunSizeId'
    & $add '        ) THEN 1'
    & $add '        ELSE 0'
    & $add '    END AS IsVisible'
    ...
    & $add 'WHERE ISNULL(srs.IsActive, 1) = 1;'
    & $add 'GO'
    & $add 'PRINT N''View_TchpSimpleQcSelectedSizes created/altered.'';'
    & $add 'GO'
