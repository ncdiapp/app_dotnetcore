...
DECLARE @PlmTemplateId     INT           = 3292;                   -- <<< USER SETTING â€” scope refs: pdmProductTemplate.TemplateID
...
WHERE m.[FieldKind] IN (N''TabField'', N''GridColumn'')
  AND m.[AppTableName] IN (N''@P@Style_Header_V2K_ERP'', N''@P@QC'', N''@P@Testing'', N''@P@SimpleQC'', N''@P@SimpleQCResult'', N''@P@Product_Test_Grid_reg'')
GROUP BY
    m.[AppTableName],
    m.[FieldKind],
    m.[DwTableName],
...
BEGIN TRY
    ...
    BEGIN
        ...
        BEGIN
            -- Multi-DW â†’ one APP table: ensure row, then UPDATE columns from this DW source.
