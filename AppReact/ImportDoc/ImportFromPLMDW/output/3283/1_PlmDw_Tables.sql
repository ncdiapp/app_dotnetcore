...
IF OBJECT_ID(N'dbo.' + QUOTENAME(@TableName), N'U') IS NULL
BEGIN
    SET @sql = N'CREATE TABLE dbo.' + QUOTENAME(@TableName) + N' ([ReferenceId] INT NOT NULL, [Spec_Selected_Size] [nvarchar](4000) NULL, [SpecSampleSize] [int] NULL, [SpecSampleColor] [int] NULL, [SpecFitStatus] [int] NULL, CONSTRAINT [PK_Fit_Summary] PRIMARY KEY CLUSTERED ([ReferenceId]) );';
    EXEC sp_executesql @sql;
END
ELSE
BEGIN
...
-- FitRoundInfo
SET @TableName = @TablePrefix + N'FitRoundInfo';
SET @RootTable = @TablePrefix + @RootTableSuffix;
SET @FkName = N'FK_' + @TableName + N'_Reference';
...
PRINT N'FX1 FitRoundInfo: no ReferenceId FK (PK = FitRoundId â†’ TchpFitRound).';
