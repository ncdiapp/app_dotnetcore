...
IF OBJECT_ID(N'dbo.' + QUOTENAME(@TableName), N'U') IS NULL
BEGIN
    SET @sql = N'CREATE TABLE dbo.' + QUOTENAME(@TableName) + N' ([ReferenceId] INT NOT NULL, [Bulk_Fabric_Status] [int] NULL, [Bulk_Fabric_Approved_Date] [datetime] NULL, [Production_Date] [datetime] NULL, [Lot__5200] [nvarchar](255) NULL, [Release_to_Manufacturer_5201] [nvarchar](255) NULL, [Lot__5202] [nvarchar](255) NULL, [Release_to_Manufacturer_5203] [nvarchar](255) NULL, [Quality_Standards] [nvarchar](255) NULL, [QA_Status] [int] NULL, [Approved_by] [int] NULL, [Comments] [nvarchar](255) NULL, [Standard_Bodies] [int] NULL, [Standard_Claim] [int] NULL, [Certification_Year] [nvarchar](255) NULL, CONSTRAINT [PK_Testing____Compliance] PRIMARY KEY CLUSTERED ([ReferenceId]) );';
    EXEC sp_executesql @sql;
END
