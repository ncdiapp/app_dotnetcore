...
- See Wijmo ComboBox selection rule in WijmoComboBoxCascadingStandards.md (set selectedValue to '' then setTimeout when setting programmatically).
...
- For Wijmo ComboBox: set selectedValue to `''` first then use `setTimeout(() => { comboBox.selectedValue = value; }, 0)` when setting programmatically. See WijmoComboBoxCascadingStandards.md.
