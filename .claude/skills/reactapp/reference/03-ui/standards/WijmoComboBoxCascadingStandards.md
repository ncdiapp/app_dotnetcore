...
5. **Child list refresh**  
   When the parent changes, clear the child list (e.g. `setTableColumns([])`) then set the new list after the API returns, so the UI clearly switches to the new options. Optionally re-apply selection in `setTimeout(..., 0)` (set selectedValue to `''` first, then set the actual value) to avoid Wijmo ComboBox binding issues.
...
- **Related rules**:  
  - ComboBox: set `selectedValue = ''` then `setTimeout(() => { comboBox.selectedValue = value; }, 0)` when setting programmatically. FlexGrid: access via ref `.control`.  
