...
- Use: `w-1 flex-auto` (this child item has at least width 1, but will expand to fill the remaining width)
- This makes the element take up the remaining horizontal space in a row layout.
- Example: `<div className="flex h-full w-full"><div className="flex-auto w-1 overflow-hidden">...</div></div>`

## Vertical Remaining Space

**When parent is `flex flex-col` **:
- Use: `h-1 flex-auto` (this child item has at least height 1, but will expand to fill the remaining height)
- This makes the element take up the remaining vertical space in a column layout.
- Example: `<div className="flex flex-col h-full w-full"><div className="flex-auto h-1 overflow-hidden">...</div></div>`
