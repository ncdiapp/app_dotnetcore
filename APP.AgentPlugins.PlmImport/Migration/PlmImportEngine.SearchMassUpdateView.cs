    ...
    public static partial class PlmImportEngine
    {
            ...
            PlmSearchMassUpdateViewLoadRequestDto request)
        {
            ...
            catch (Exception ex)
            {
                result.ValidationResult.Items.Add(new ValidationItem(
                    typeof(PlmImportEngine), "Plm_SearchMassUpdate_Load_Error", ValidationItemType.Error, ex.Message));
            }
            ...
            PlmSearchMassUpdateViewBlueprintDto blueprint)
        {
            ...
            catch (Exception ex)
            {
                ...
                result.ValidationResult.Items.Add(new ValidationItem(
                    typeof(PlmImportEngine), "Plm_SearchMassUpdate_Validate_Error", ValidationItemType.Error, ex.Message));
            }
            ...
            PlmSearchMassUpdateViewBlueprintDto blueprint)
        {
            ...
            catch (Exception ex)
            {
                ...
                result.ValidationResult.Items.Add(new ValidationItem(
                    typeof(PlmImportEngine), "Plm_SearchMassUpdate_Preview_Error", ValidationItemType.Error, ex.Message));
            }
            ...
            PlmSearchMassUpdateViewExecuteRequestDto request)
        {
            ...
            catch (Exception ex)
            {
                ...
                result.Object.ErrorMessage = ex.Message;
                result.ValidationResult.Items.Add(new ValidationItem(
                    typeof(PlmImportEngine), "Plm_SearchMassUpdate_Execute_Error", ValidationItemType.Error, ex.Message));
            }
            ...
            PlmSearchMassUpdateViewExecuteResultDto executeResult)
        {
                ...
                && le.Create != null)
            {
                ...
                int childCount = (le.Create.UnitStructure?.Children ?? new List<PlmSearchMassUpdateListEditUnitDto>())
                    .Count(c => !string.IsNullOrWhiteSpace(c?.AppTableName));
                int? saasId = le.Create.SaasApplicationId ?? saasApplicationId;

                var pack = MassUpdateListEditAppConfigPackBuilder.Build(le.Create, saasId);
                ...
                {
                    throw new InvalidOperationException(
                        packExec.Object?.ErrorMessage
                        ?? packExec.ValidationResult?.Items?.FirstOrDefault()?.Message
                        ?? "AppConfigPack execute failed for Mass Update ListEdit.");
                }
                ...
                int txId = GetTransactionIdByIntegrationId(conn, null, integrationId)
                    ?? throw new InvalidOperationException(
                        $"ListEdit '{integrationId}' was not created by AppConfigPack.");

                // Pack sets OrganizedType=List + field ControlType/Entity/LinkToParent; re-apply
                // blueprint FK links for PLM tables that share ReferenceId without a schema FK.
                ...
                executeResult.Messages.Add(
                    $"Created ListEdit Transaction #{txId} IntegrationId={integrationId} via AppConfigPack (OrganizedType=List), root={rootTable}, children={childCount}.");
                executeResult.ListEditIntegrationId = integrationId;
                return txId;
            }
