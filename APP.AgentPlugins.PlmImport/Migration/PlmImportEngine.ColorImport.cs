    ...
    public static partial class PlmImportEngine
    {
        ...
        public static OperationCallResult<PlmColorImportPreviewDto> PreviewPlmColorImport(int? sessionId)
        {
            ...
            try
            {
                ...
                if (!result.Object.IsSuccess)
                {
                    result.ValidationResult.Items.Add(new ValidationItem(
                        typeof(PlmImportEngine), "Plm_Color_Preview_Error", ValidationItemType.Error,
            ...
            catch (Exception ex)
            {
                ...
                result.ValidationResult.Items.Add(new ValidationItem(
                    typeof(PlmImportEngine), "Plm_Color_Preview_Error", ValidationItemType.Error, ex.Message));
            }
        ...
        public static OperationCallResult<PlmColorImportExecuteResultDto> ExecutePlmColorImport(PlmColorImportExecuteRequestDto request)
        {
            ...
            try
            {
                ...
                string plmConn = RequireResolvedPlmConnection(session);
                ...
                {
                    result.ValidationResult.Items.Add(new ValidationItem(
                        typeof(PlmImportEngine), "Plm_Color_Execute_Error", ValidationItemType.Error,
                        result.Object.ErrorMessage));
                }
                else
                {
            ...
            catch (Exception ex)
            {
                ...
                result.Object.ErrorMessage = ex.Message;
                result.ValidationResult.Items.Add(new ValidationItem(
                    typeof(PlmImportEngine), "Plm_Color_Execute_Error", ValidationItemType.Error, ex.Message));
            }
            ...
            int tenantDataSourceId)
        {
            ...
            using (var tenantConn = new SqlConnection(tenantConnectionString))
            {
                ...
                int transactionId;
                int listSearchId;
                EnsureRgbColorAppConfigViaPack(
                    tenantConn, saasApplicationId, columns, executeResult, out transactionId, out listSearchId);
...
WHERE FolderID = @FolderId
  AND ISNULL(ParentID, -1) <> @ParentId;";
        ...
        private static void EnsureRgbColorAppConfigViaPack(
            SqlConnection tenantConn,
            int? saasApplicationId,
            List<ColorColumnRow> columns,
            PlmColorImportExecuteResultDto executeResult,
            out int transactionId,
            out int listSearchId)
        {
            var colNames = (columns ?? new List<ColorColumnRow>())
                .Where(c => c != null && !string.IsNullOrWhiteSpace(c.ColumnName))
                .Select(c => c.ColumnName)
                .ToList();

            var pack = ColorImportAppConfigPackBuilder.Build(saasApplicationId, colNames);
            ...
            {
                throw new InvalidOperationException(packExec.Object?.ErrorMessage
                    ?? packExec.ValidationResult?.Items?.FirstOrDefault()?.Message
                    ?? "AppConfigPack execute failed for RGB Color.");
            }

            if (packExec.Object.Messages != null)
                executeResult.Messages.AddRange(packExec.Object.Messages);

            transactionId = GetTransactionIdByIntegrationId(tenantConn, null, ColorTxIntegrationId)
                ?? throw new InvalidOperationException("RGB Color transaction was not created by AppConfigPack.");
            listSearchId = GetSearchIdByIntegrationId(tenantConn, null, ColorListSearchIntegrationId)
                ?? throw new InvalidOperationException("RGB Color list search was not created by AppConfigPack.");

            EnsureTransactionQuickSearchNavigation(tenantConn, transactionId, listSearchId);
            executeResult.Messages.Add($"RGB Color TX {transactionId} + list search {listSearchId} via AppConfigPackBL.");
        }
