    ...
    public static partial class PlmImportEngine
    {
        ...
        public static OperationCallResult<PlmPomImportPreviewDto> PreviewPlmPomImport(int? sessionId)
        {
            ...
            try
            {
                ...
                if (!result.Object.IsSuccess)
                {
                    result.ValidationResult.Items.Add(new ValidationItem(
                        typeof(PlmImportEngine), "Plm_Pom_Preview_Error", ValidationItemType.Error,
            ...
            catch (Exception ex)
            {
                ...
                result.ValidationResult.Items.Add(new ValidationItem(
                    typeof(PlmImportEngine), "Plm_Pom_Preview_Error", ValidationItemType.Error, ex.Message));
            }
        ...
        public static OperationCallResult<PlmPomImportExecuteResultDto> ExecutePlmPomImport(PlmPomImportExecuteRequestDto request)
        {
            ...
            try
            {
                ...
                {
                    result.ValidationResult.Items.Add(new ValidationItem(
                        typeof(PlmImportEngine), "Plm_Pom_Execute_Error", ValidationItemType.Error,
                        result.Object.ErrorMessage));
                }
                else
                {
            ...
            catch (Exception ex)
            {
                ...
                result.Object.ErrorMessage = ex.Message;
                result.ValidationResult.Items.Add(new ValidationItem(
                    typeof(PlmImportEngine), "Plm_Pom_Execute_Error", ValidationItemType.Error, ex.Message));
            }
            ...
            bool importFoldersIfMissing)
        {
            ...
            using (var tenantConn = new SqlConnection(tenantConnectionString))
            {
                ...
                int pomTransactionId;
                int pomListSearchId;
                EnsurePomBodyPartAppConfigViaPack(
                    tenantConn, saasApplicationId, pomColumns, executeResult,
                    out pomTransactionId, out pomListSearchId);
                executeResult.PomTransactionId = pomTransactionId;
                executeResult.PomListSearchId = pomListSearchId;
...
ORDER BY CASE WHEN SessionId = @SessionId THEN 0 ELSE 1 END, LastSyncAt DESC, MapId DESC;";
        ...
        private static void EnsurePomBodyPartAppConfigViaPack(
            SqlConnection tenantConn,
            int? saasApplicationId,
        ...
        {
            var colNames = (columns ?? new List<PomColumnRow>())
                .Where(c => c != null && !string.IsNullOrWhiteSpace(c.ColumnName))
                .Select(c => c.ColumnName)
                .ToList();

            var pack = PomImportAppConfigPackBuilder.BuildBodyPart(saasApplicationId, colNames);
            ...
            if (packExec.Object == null || !packExec.Object.IsSuccess)
            {
                throw new InvalidOperationException(packExec.Object?.ErrorMessage
                    ?? "AppConfigPack execute failed for POM Body Part.");
            }

            if (packExec.Object.Messages != null)
                executeResult.Messages.AddRange(packExec.Object.Messages);

            transactionId = GetTransactionIdByIntegrationId(tenantConn, null, PomTxIntegrationId)
                ?? throw new InvalidOperationException("POM Body Part transaction missing after AppConfigPack.");
            listSearchId = GetSearchIdByIntegrationId(tenantConn, null, PomListSearchIntegrationId)
                ?? throw new InvalidOperationException("POM list search missing after AppConfigPack.");

            EnsureTransactionQuickSearchNavigation(tenantConn, transactionId, listSearchId);
            executeResult.Messages.Add($"POM Body Part TX {transactionId} + list search {listSearchId} via AppConfigPackBL.");
        }
