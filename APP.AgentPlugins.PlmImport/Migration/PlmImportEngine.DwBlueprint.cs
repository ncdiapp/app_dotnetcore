    ...
    public static OperationCallResult<PlmDwImportBlueprintDto> LoadDwImportBlueprint(PlmDwBlueprintLoadRequestDto request)
    {
      ...
      catch (Exception ex)
      {
        result.ValidationResult.Items.Add(new ValidationItem(
          typeof(PlmImportEngine), "Plm_DwBlueprint_Load_Error", ValidationItemType.Error, ex.Message));
      }
...
WHERE BlueprintKey = @BlueprintKey";
      ...
      {
        result.ValidationResult.Items.Add(new ValidationItem(
          typeof(PlmImportEngine), "Plm_DwBlueprint_LoadTable_Error", ValidationItemType.Error, ex.Message));
      }
    ...
    public static OperationCallResult<PlmDwBlueprintValidationDto> ValidateDwImportBlueprint(PlmDwImportBlueprintDto blueprint)
    {
      ...
      catch (Exception ex)
      {
        ...
        result.ValidationResult.Items.Add(new ValidationItem(
          typeof(PlmImportEngine), "Plm_DwBlueprint_Validate_Error", ValidationItemType.Error, ex.Message));
      }
    ...
    public static OperationCallResult<PlmDwBlueprintPreviewDto> PreviewDwBlueprintConfig(PlmDwImportBlueprintDto blueprint)
    {
      ...
      try
      {
        ...
        {
          SaasApplicationId = blueprint.TransactionGroup?.SaasApplicationId,
      ...
      catch (Exception ex)
      {
        ...
        result.ValidationResult.Items.Add(new ValidationItem(
          typeof(PlmImportEngine), "Plm_DwBlueprint_Preview_Error", ValidationItemType.Error, ex.Message));
      }
    ...
    public static OperationCallResult<PlmDwBlueprintExecuteResultDto> ExecuteDwBlueprintConfig(PlmDwBlueprintExecuteRequestDto request)
    {
      ...
      try
      {
        ...
        var packOptions = new DwBlueprintAppConfigPackBuilder.Options
        {
          ...
          IncludeNavigation = request.IncludeNavigation,
        ...
        if (packExec.Object == null || !packExec.Object.IsSuccess)
        {
          ...
          result.Object.ErrorMessage = packExec.Object?.ErrorMessage
            ?? packExec.ValidationResult?.Items?.FirstOrDefault()?.Message
            ?? "AppConfigPack execute failed.";
          if (packExec.Object?.Messages != null)
          {
            foreach (var msg in packExec.Object.Messages)
              result.Object.Messages.Add(msg);
          }
          return result;
        }

        result.Object.TransactionsInserted = packExec.Object.TransactionsInserted;
        result.Object.TransactionsUpdated = packExec.Object.TransactionsUpdated;
        result.Object.TransactionGroupId = packExec.Object.TransactionGroupId;
        if (packExec.Object.Messages != null)
          result.Object.Messages.AddRange(packExec.Object.Messages);
        result.Object.Messages.Add("Applied via AppConfigPackBL (pivots / FitRound links in pack).");

        // Residual PLM cleanup only: delete BOM colorway staging host fields CreateHierarchy may add.
        ...
        {
          conn.Open();
          ApplyDwBlueprintStagingCleanup(conn, request.Blueprint, plans, prefix, result.Object);
          ...
          // Resolve SearchId from pack search integration id
          var searchIntegrationId = request.Blueprint.SearchView?.Search?.IntegrationId;
          if (request.IncludeSearchView && !string.IsNullOrWhiteSpace(searchIntegrationId))
          {
      ...
      catch (Exception ex)
      {
        ...
        result.Object.ErrorMessage = ex.Message;
        result.ValidationResult.Items.Add(new ValidationItem(
          typeof(PlmImportEngine), "Plm_DwBlueprint_Execute_Error", ValidationItemType.Error, ex.Message));
      }
    ...
    private static HashSet<string> LoadExistingTransactionIntegrationIds(string tenantConn)
    {
      ...
      using (var conn = new SqlConnection(tenantConn))
      {
        ...
        {
          cmd.CommandText = @"
SELECT IntegrationId FROM dbo.AppTransaction
WHERE IntegrationId IS NOT NULL AND LTRIM(RTRIM(IntegrationId)) <> ''";
    ...
    /// <summary>
    /// After AppConfigPack apply: only remove BOM colorway staging host fields (Colorway_N / ImageN).
    /// Pivot GridDisplayType / IsPivot* / FitRound links are expressed in the pack.
    /// </summary>
    private static void ApplyDwBlueprintStagingCleanup(
      SqlConnection conn,
      PlmDwImportBlueprintDto blueprint,
      List<TemplateTabExecutionPlan> plans,
      string prefix,
      PlmDwBlueprintExecuteResultDto executeResult)
    {
      int cleaned = 0;
      foreach (var plan in plans)
      {
        if (plan.Tab.ImportStatus == TemplateStatusSkipped)
          continue;
        if (plan.BomColorwayPivotBindings == null || plan.BomColorwayPivotBindings.Count == 0)
          continue;

        string integrationId = ResolveDwTransactionIntegrationId(blueprint, plan);
        int? txId = GetTransactionIdByIntegrationId(conn, null, integrationId);
        if (!txId.HasValue)
          continue;

        foreach (var binding in plan.BomColorwayPivotBindings.Where(b => b != null && b.PlmTabId == plan.Tab.TabId))
        {
          if (string.IsNullOrWhiteSpace(binding.HostAppTableName))
            continue;
          string hostTable = QualifyBlueprintTableName(binding.HostAppTableName, prefix, skipTablePrefix: false);
          int? hostUnitId = GetTransactionUnitIdByTableName(conn, null, txId.Value, hostTable);
          if (!hostUnitId.HasValue)
            continue;
          DeleteHostStagingPivotFields(conn, null, hostUnitId.Value, binding);
          cleaned++;
        }
      }

      if (cleaned > 0)
        executeResult.Messages.Add($"Cleaned BOM colorway staging host fields on {cleaned} binding(s).");
    }
    ...
    public static OperationCallResult<bool> RefreshDwImportTenantCaches(PlmDwRefreshCachesRequestDto request)
    {
      ...
      {
        result.ValidationResult.Items.Add(new ValidationItem(
          typeof(PlmImportEngine), "Plm_DwBlueprint_RefreshCaches_Error", ValidationItemType.Error, ex.Message));
      }
