...
namespace ExchangeBL
{
    public class FtpTools
    {
        ...
        /* Construct Object */
        public FtpTools(string hostIP, string userName, string password) { host = hostIP; user = userName; pass = password; }

        /* Download File */
        public void download(string ftpFileFullPath, string destinationFilePath, out string errorMsg)
        {
            errorMsg = "";

            string srcFileName = Path.GetFileName(ftpFileFullPath);
          
            if (string.IsNullOrWhiteSpace(srcFileName))
            {
                errorMsg = "Cannot get file name from ftp file path: " + ftpFileFullPath;
            }
            else
            {
                ...
                if (string.IsNullOrWhiteSpace(targetFileName))
                {
                    ...
                    destinationFilePath = System.IO.Path.Combine(destinationFilePath, targetFileName);
                }
                else 
                {
                    ...
                    {
                        destinationFilePath = destinationFilePath.Substring(0, destinationFilePath.Length - targetExtension.Length) + srcExtension;
                    }
                }
                ...
                try
                {
                    ...
                    ftpRequest = (FtpWebRequest)FtpWebRequest.Create(ftpFileFullPath);
                    ...
                    FileStream localFileStream = new FileStream(destinationFilePath, FileMode.Create);
        ...
        public byte[] GetFtpFileBinaryData(string ftpFileFullPath, out string errorMsg)
        {
            errorMsg = "";
            try
            {
        ...
        {
            FtpTools ftpClient = new FtpTools(@"ftp://10.10.10.10/", "user", "password");
