...
{
    /// <summary>
    /// Core agent orchestrator for the Report AI agent.
    ///
    /// Mirrors AppBuilderAgentBL's architecture:
    ///   Plugin discovery via [AgentFunction] attributes → build LLM tool definitions
    ///   Agentic loop: LLM → tool_use → reflection invoke → feed result back
    ///   Done callback carries AgentReportDoneEvent with the search result
    ///
    /// Tools exposed:
    ///   ReportSearchPlugin  — list_available_searches, get_search_criteria, execute_report
    ///   PlatformExplorerPlugin — list_applications (needed for build fallback)
    ///   SearchBuilderPlugin    — create_search (builds a new search when no match found)
    /// </summary>
    public static class AppReportAgentBL
    {
...
━━━ RULES ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ...
  - Keep your final response concise: state what was found and any applied filters.";
        ...
        public static async Task RunAgentAsync(
            AppReportAgentRequestDto request,
            ReportAgentCallbacks callbacks,
            AppClientIdentity? agentIdentity = null)
        {
            try
            {
                // Register system agent identity (same pattern as AppBuilderAgentBL)
                agentIdentity = RegisterSystemAgentIdentity(agentIdentity);

                // 1. Discover tools
                var reportPlugin = new ReportSearchPlugin();
                var tools = DiscoverTools(reportPlugin, request.DataSourceRegisterId);

                // 2. Build LLM tool definitions
                ...
                // 5. Build done payload
                var updatedHistory = BuildUpdatedHistory(request, finalResponse);

                await SafeCallback(callbacks.OnDone, new AgentReportDoneEvent
                {
                    FinalResponse  = finalResponse,
                    ReportResult   = reportPlugin.LastReportResult,
        ...
        private static List<ToolDescriptor> DiscoverTools(
            ReportSearchPlugin reportPlugin, int? dataSourceRegisterId)
        {
            var pluginInstances = new List<object> { reportPlugin };

            // PlatformExplorerPlugin provides list_applications (needed for build fallback)
            // SearchBuilderPlugin provides create_search
            if (dataSourceRegisterId.HasValue)
            {
                pluginInstances.Add(new PlatformExplorerPlugin(dataSourceRegisterId));
                pluginInstances.Add(new SearchBuilderPlugin(dataSourceRegisterId));
            }
        ...
        // Copied from AppBuilderAgentBL — private to this class to stay self-contained
        // ─────────────────────────────────────────────────────────────────────
        ...
        private static string FriendlyLabel(string n)
        {
            switch (n)
            {
                case "list_available_searches": return "Listing available searches…";
                case "get_search_criteria":     return "Getting search criteria fields…";
                case "execute_report":          return "Executing report…";
                case "list_applications":       return "Listing applications…";
                case "create_search":           return "Building new report/search screen…";
                default:                        return n;
            }
        }
        ...
        private static List<AppReportAgentMessageDto> TrimConversationHistory(
            List<AppReportAgentMessageDto> history)
        {
        ...
        private static List<AppReportAgentMessageDto> BuildUpdatedHistory(
            AppReportAgentRequestDto request, string finalResponse)
        {
            var history = new List<AppReportAgentMessageDto>(
                request.ConversationHistory ?? new List<AppReportAgentMessageDto>());
            history.Add(new AppReportAgentMessageDto { Role = "user",      Content = request.UserMessage });
            if (!string.IsNullOrEmpty(finalResponse))
                history.Add(new AppReportAgentMessageDto { Role = "assistant", Content = finalResponse });
            return history;
        }
