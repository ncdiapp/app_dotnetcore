...
- `StyleSpecId` — PK on `TchpStyleSpec` (non-identity); equals product `ReferenceId` (sibling link; no `ProductReferenceId` column). Same on `TchpQcOrder`: product scope is `StyleSpecId` only (no `ProductReferenceId`).
