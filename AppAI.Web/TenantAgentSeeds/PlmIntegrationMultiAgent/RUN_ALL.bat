...
sqlcmd -S "%SERVER%" -d "%DB%" -E -b -f 65001 -i "%HERE%00_Ensure_PlatformMultiAgent_AskUser.sql" || goto fail
sqlcmd -S "%SERVER%" -d "%DB%" -E -b -f 65001 -i "%HERE%01_Seed_IntegrationPlmImportLibrary.sql" || goto fail
sqlcmd -S "%SERVER%" -d "%DB%" -E -b -f 65001 -i "%HERE%02_Seed_PlatformAppConfigPackLibrary.sql" || goto fail
sqlcmd -S "%SERVER%" -d "%DB%" -E -b -f 65001 -i "%HERE%03_Seed_AppConfigPackOrchestrator.sql" || goto fail
sqlcmd -S "%SERVER%" -d "%DB%" -E -b -f 65001 -i "%HERE%04_Seed_PlmIntegrationImportDw_Child.sql" || goto fail
sqlcmd -S "%SERVER%" -d "%DB%" -E -b -f 65001 -i "%HERE%05_Seed_PlmIntegrationOrchestrator_Root.sql" || goto fail
