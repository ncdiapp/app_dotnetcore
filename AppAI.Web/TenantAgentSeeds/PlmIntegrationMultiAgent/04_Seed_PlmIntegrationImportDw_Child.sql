...
IF NOT EXISTS (SELECT 1 FROM dbo.AppAgentSkillSet WHERE SkillKey = N'plm-integration-import-dw')
...
## Non-negotiable runtime rules (override anything below)
1. ExecutionMode=**Deterministic**. **Never** ask the user questions. **Never** wait for confirmation. **Never** STOP for HITL.
2. If inputs are missing: return a structured error / checklist for ROOT - do not invent TemplateId or DataSourceId.
3. On start, read shared context keys (if present):
   - `plm.integration.import-dw.inputs`
   - `plm.integration.import-dw.plan` (required for PHASE=B)
   - `plm.integration.job`
...
- `plm.integration.import-dw.inputs`
- `plm.integration.import-dw.phase-a`
- `plm.integration.import-dw.plan`
- `plm.integration.import-dw.outputs`
...
> **Runtime (this file):** AppAI **Agent Management** GenericAgent. Probes use BuiltIn tools `execute_sql` / `get_table_schema` / `get_database_tables` / `get_database_schema` with optional `dataSourceId` (DataSourceRegisterId). Do **not** use sqlcmd or raw connection strings.
> **Files:** chat **Files** panel + BuiltIn library `agent-files` (`file_list` / `file_read` / `file_write` / `file_mkdir` / `file_delete`). Paths are **relative** to the chat file root (see §Agent Management file area). Do **not** invent absolute `FileRepository\...` paths in tools - use `source/...` and `output/...`.
> **Scripts:** library `agent-scripts` tool `run_agent_script` runs user-uploaded `.ps1` under `source/` on the App server (sqlcmd via official generator). Subscribe this library on the PLM agent.
> **Do not use:** Cursor IDE-only file writes, `appai` MCP, or connection-string Gate 0 from the original PROMPT.md - this file replaces those with DataSourceId.

---

## Agent Management file area (mandatory)

**Disk mapping (for humans / debugging only - do not hardcode CompanyId or SessionKey in tools):**

```text
{AppAI.Web}/FileRepository/Company_{CompanyId}/AgentOutput/{SessionKey}/
  source/     ← official generators + templates + dwTabImportConfig.json
  output/     ← Phase B deliverables (output/{templateId}/1_…6_)
```

Example (one tenant / one fixed chat):  
`FileRepository/Company_5042/AgentOutput/plm-integration-dw3_14797/`  
(`SessionKey` = `SkillKey:UserId`, `:` sanitized to `_` on disk.)

**Agent must use relative paths via file tools:**
...
### Forbidden Phase B shortcuts (root cause of Validate failures)

| Forbidden | Why it fails |
|-----------|----------------|
| Hand-writing a tiny `4_PlmDw_ImportBlueprint.json` with top-level `appTable` / `unitType` on each transaction | BL Validate needs `unitStructure.siblingUnits` / `childUnits` (+ `rootUnit`, `blueprintFields`, …). Config fields ≠ execute Blueprint. |
...
```text
1. PLM DataSourceId (DataSourceRegisterId for PLM source DB - pdmTemplate, pdmTemplateTab, pdmProductTemplate)
   e.g. 12

2. plmDW DataSourceId (DataSourceRegisterId for plmDW)
   e.g. 13
...
**Tool rule:** After Gate 0, every probe MUST call `execute_sql` (or `get_table_schema` / `get_database_tables`) with the matching `dataSourceId`. Never claim probe results without a tool_result. Omit `dataSourceId` only when intentionally querying the session default APP tenant DB.
...
1. **STOP immediately.** Do **not** call tools, probe SQL, Phase A analysis, or Phase B generation.
2. **Ask the user** for PLM DataSourceId + plmDW DataSourceId + **one** TemplateId (see §Example session message).
3. **Do not** treat any of the following as user input:
   - Example DataSourceIds or TemplateId in §Example session message
...
**Prerequisite:** Gate 0 passed - user supplied **PLM DataSourceId**, **plmDW DataSourceId**, and **one TemplateId**.

Store as `@PlmDataSourceId` and `@DwDataSourceId` (integers). Connection strings are resolved by AppAI DataSourceRegister - **never ask for or log passwords**.

### A1b. Load template + tab list from PLM (authoritative)

Call `execute_sql` with `dataSourceId=@PlmDataSourceId`. Use the SQL from `source/_plm_probe_template.sql` (or the equivalent SELECT below), with `@TemplateId` substituted in the SQL text.
...
This PROMPT is used in **three** places. **Detect which one you are in, then follow only that branch.** Prefer **B0-AGENT** when `file_list` / `file_write` (library `agent-files`) and/or `run_agent_script` (library `agent-scripts`) are available.
...
```json
{
  ...
  "plmDatabase": "PLM",
  "dwDatabase": "plmDW",
  "_note_AppAgent": "Prefer plmDataSourceId/dwDataSourceId. sqlServer/connection strings are Cursor IDE / generator only.",
...
**Agent Management (B0-AGENT):**

1. Require official files under chat `source/` (user upload via **Files** if missing).
2. `file_write` `source/dwTabImportConfig.json` with DataSourceIds.
3. Call **`run_agent_script`** (`relativePath=source/_gen_plmdw_import_sql.ps1`) - App host runs official PS + sqlcmd. Forbidden: hand-written stub Blueprint / DDL.
4. Verify with `file_list` / `validate_agent_outputs` (SizeBytes + `unitStructure`).
...
PLM DataSourceId + plmDW DataSourceId + **new** TemplateId only. Tab list comes from PLM again.
...
### Repo / Cursor IDE (`ImportFromPLMDW/`)

```text
ImportFromPLMDW/
  PROMPT.md / Prompt_AppAgent.txt
  output/{templateId}/1_…6_
  source/   ← official generators + templates (copy these into Agent Management Files)
```

### Agent Management chat Files root

```text
AgentOutput/{SessionKey}/          e.g. …/Company_5042/AgentOutput/plm-integration-dw3_14797/
  source/
    _gen_plmdw_import_sql.ps1      ← REQUIRED before Phase B
...
**Insufficient** (agent must ask for the three required items (two DataSourceIds + TemplateId)):
...
PLM DataSourceId: 12
plmDW DataSourceId: 13
TemplateId to import: 3359
```

Agent loads TabIds from `pdmTemplateTab` for TemplateId 3359 - user does **not** list TabIds.
...
SET DisplayName = N'PLM Integration Import DW',
    Description = N'Deterministic child: ImportFromPLMDW execute worker; HITL owned by orchestrator',
    SystemPrompt = N'# CHILD WORKER - SkillKey: `plm-integration-import-dw`
# Parent ROOT: `plm-integration-orchestrator` (only agent that talks to the user)
...
AND NOT EXISTS (
    ...
    WHERE SkillKey = N'plm-integration-import-dw' AND LibraryKey = N'platform-multi-agent')
INSERT INTO dbo.AppAgentLibrarySubscription (SkillKey, LibraryKey)
VALUES (N'plm-integration-import-dw', N'platform-multi-agent');
GO
...
AND NOT EXISTS (
    SELECT 1 FROM dbo.AppAgentLibrarySubscription
    WHERE SkillKey = N'plm-integration-import-dw' AND LibraryKey = N'agent-files')
INSERT INTO dbo.AppAgentLibrarySubscription (SkillKey, LibraryKey)
VALUES (N'plm-integration-import-dw', N'agent-files');
GO

IF EXISTS (SELECT 1 FROM dbo.AppAgentSkillSet WHERE SkillKey = N'plm-integration-import-dw')
AND EXISTS (SELECT 1 FROM dbo.AppAgentToolLibrary WHERE LibraryKey = N'agent-scripts')
AND NOT EXISTS (
    SELECT 1 FROM dbo.AppAgentLibrarySubscription
    WHERE SkillKey = N'plm-integration-import-dw' AND LibraryKey = N'agent-scripts')
INSERT INTO dbo.AppAgentLibrarySubscription (SkillKey, LibraryKey)
VALUES (N'plm-integration-import-dw', N'agent-scripts');
GO
...
AND NOT EXISTS (
    SELECT 1 FROM dbo.AppAgentLibrarySubscription
    WHERE SkillKey = N'plm-integration-import-dw' AND LibraryKey = N'platform-database')
INSERT INTO dbo.AppAgentLibrarySubscription (SkillKey, LibraryKey)
VALUES (N'plm-integration-import-dw', N'platform-database');
GO
...
AND NOT EXISTS (
    SELECT 1 FROM dbo.AppAgentLibrarySubscription
    WHERE SkillKey = N'plm-integration-import-dw' AND LibraryKey = N'platform-application')
INSERT INTO dbo.AppAgentLibrarySubscription (SkillKey, LibraryKey)
VALUES (N'plm-integration-import-dw', N'platform-application');
GO

IF EXISTS (SELECT 1 FROM dbo.AppAgentSkillSet WHERE SkillKey = N'plm-integration-import-dw')
AND EXISTS (SELECT 1 FROM dbo.AppAgentToolLibrary WHERE LibraryKey = N'platform-memory')
AND NOT EXISTS (
    SELECT 1 FROM dbo.AppAgentLibrarySubscription
    WHERE SkillKey = N'plm-integration-import-dw' AND LibraryKey = N'platform-memory')
INSERT INTO dbo.AppAgentLibrarySubscription (SkillKey, LibraryKey)
VALUES (N'plm-integration-import-dw', N'platform-memory');
GO

IF EXISTS (SELECT 1 FROM dbo.AppAgentSkillSet WHERE SkillKey = N'plm-integration-import-dw')
AND EXISTS (SELECT 1 FROM dbo.AppAgentToolLibrary WHERE LibraryKey = N'platform-search')
AND NOT EXISTS (
    SELECT 1 FROM dbo.AppAgentLibrarySubscription
    WHERE SkillKey = N'plm-integration-import-dw' AND LibraryKey = N'platform-search')
INSERT INTO dbo.AppAgentLibrarySubscription (SkillKey, LibraryKey)
VALUES (N'plm-integration-import-dw', N'platform-search');
GO
