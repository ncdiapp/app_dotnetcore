...
**Callers:** Import Config UI / AI → `Execute`. PLM Ex DLL (`APP.AgentPlugins.PlmImport`) → compose `AppConfigPackDto` via builders → `Execute` (or `BeginSteps` + selected steps). Do **not** put PLM / Blueprint / FieldMapping concepts into AppConfigPackBL.
